...
var mess_generate =
`
// @Generated by DreamScript 0.6.8
...
function compileDir(startPath, filter) {
    ...
    for (var i = 0; i < files.length; i++) {
        ...
        if (stat.isDirectory()) {
            ...
            fs.writeFileSync(saves, mess_generate + result)
            })
        };
    };
};
...
app.command('c <file>')
    ...
    .action(filepath => {
        ...
        ds_exports.compile(ds_exports.getRealPath(filepath + '.ds'), result => {
            console.log(mess_generate + result)
        })
    })
...
.action(filepath => {
    checkOption()
    ds_exports.compile(ds_exports.getRealPath(filepath + '.ds'), result => {
        fs.writeFile(`${filepath}.js`, mess_generate + result, err => {
            if (err) {
