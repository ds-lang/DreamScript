...
exports.parse = (tokens, callback) => {
    ...
    const token_handler = {
        ...
        TUPLE: tuple_handler,
        SET: set_handler,
        SETARRAY: setarray_handler,
        REQUIRE: require_handler,
        [constant.MEDIAN]: median_handler,
        [constant.START]: start_handler
    }
...
function set_handler(token) {

    let words = token.value.split(' ')
    let words_parsed = words[0].split('(', '')

    let format = /set ([^)]*[a-zA-Z]) = ([^\[\]\(\)\n]+) = \(([^\[\]\(\)\n]+)\)/;
    let match = token.value.match(format);
    if (!match) return triggerError(`Syntax 'set' is Error`, token.line)


    return `const ${match[1]} = ${match[2]}` + `\ncheckTypes("${match[3]}", ${match[1]})`
}

function setarray_handler(token) {

    let words = token.value.split(' ')
    let words_parsed = words[0].split('(', '')

    let format = /setArray ([^)]*[a-zA-Z]): ([^\[\]\(\)\n]+) = \(([^\[\]\(\)\n]+)\)/;
    let match = token.value.match(format);
    if (!match) return triggerError(`Syntax 'setArray' is Error`, token.line)
...
function require_handler(token) {

    if(token.value === "require()") {
        return triggerError(`Syntax 'require()' is not avalaible in DreamScript`, token.line)
    }
...
function write_handler(token) {

   const parsed = token.value
        .replace('write', 'console.log')
